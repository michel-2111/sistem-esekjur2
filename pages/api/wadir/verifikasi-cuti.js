// pages/api/wadir/verifikasi-cuti.js
    ...
    try {
        ...
        if (req.method === 'GET') {
            const applications = await prisma.leave_Application.findMany({
                where: { status: 'menunggu_wadir' }, // Wadir melihat semua yang statusnya menunggu wadir
                include: {
        ...
        if (req.method === 'POST') {
            ...
            let dataToUpdate = {};
            if (action === 'approve') {
                dataToUpdate = { status: 'disetujui', alasan_ditolak: null };
