...
const style = `
    ...
    // 1. Tambahkan label Kepala Lab di sini
    const ROLE_LABELS = {
    dosen: 'Dosen',
    kaprodi: 'Kaprodi',
    wadir: 'Wadir',
    ...
    // 2. Tambahkan role KEPALA_LAB ke dalam array tombol filter
    const rolesForBulkSelect = ['dosen', 'kaprodi', 'wadir', 'p4m', 'KEPALA_LAB'];

    // 3. Tambahkan .toLowerCase() pada pengecekan agar kebal terhadap perbedaan kapitalisasi
    const handleSelectRole = (role, checked) => {
        const ids = recipientList
        .filter(u => u.roles.some(r => r.role.nama_role.toLowerCase() === role.toLowerCase()))
        .map(u => u.id);
